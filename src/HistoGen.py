...
def build_histograms(df, writeFiles=False, outdir=".",
    ...
    for h in df.ID.unique():
        ...
        f = plt.figure(figno, figsize=(6, 4), dpi=80,
                       facecolor='w', edgecolor='k')
        # since our histo data is already binned, use the weighted version to
        # reproduce it
        plt.hist(df[df.ID == h].binlabel, bins=df[df.ID == h].binMin,
                 weights=df[df.ID == h].value)
        ...
        if(writeFiles):
            ...
            plt.savefig(outdir + '\\Fig_' + str(figno) + '.png',
                        bbox_inches='tight')
            if(verbose):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog="HistoGen", description='Generate '
                                     'some MadAnalysis5 histograms from an '
                                     'associated SAF file.')
    parser.add_argument('infile', type=str,
                        help='Path to a a valid SAF file describing '
                             'histogram data')
    parser.add_argument('-o', '--outdir', type=str, action='store',
                        help='Location in which to store generated histograms '
                        '((default is current dir)')
    parser.add_argument('--verbose',  action='store_true',
                        help='Provide lots of output during run '
                             '(default is silent)')
    parser.add_argument('-t', '--terse', action='store_true',
                        help='Provide some  output during run '
                             '(default is silent)')
    ...
    figlist = build_histograms(safDf, writeFiles=True, outdir=od,
                               verbose=args.verbose, terse=args.terse)

    # Example of using returned collection of tuples
